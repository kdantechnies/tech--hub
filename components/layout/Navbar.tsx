...
export const Navbar = () => {
  ...
  // Robust check: Is the user exactly on the homepage?
  ...
  return (
    <nav className={`fixed top-0 w-full z-[100] transition-all duration-500 ${
      ...
      {/* Container with px-6 to ensure mobile icons don't hit the screen edge */}
      <div className="max-width-container flex items-center justify-between px-6">
        ...
        <Link href="/" onClick={closeMenu} className="flex items-center gap-3 group shrink-0">
          <div className="relative w-10 h-10 md:w-11 md:h-11 transition-transform group-hover:rotate-12 duration-300">
            <Image src="/logo.png" alt="Jengatech Logo" fill className="object-contain" priority sizes="44px" />
          </div>
        ...
        {/* --- MOBILE ACTION AREA (Outside Toggle Menu) --- */}
        <div className="flex items-center gap-3 md:hidden">
          
          {/* FAQ ICON: Only visible on mobile, and NOT on the home page */}
          {!isHomePage && (
            <Link 
              href="/faq" 
              onClick={closeMenu}
              className="flex items-center justify-center w-10 h-10 rounded-xl bg-orange-50 text-[#FF6B00] border border-orange-100 shadow-sm active:scale-90 transition-all"
            >
          ...
          {/* HAMBURGER TOGGLE */}
          <button 
            className="flex items-center justify-center w-10 h-10 text-[#0A0A0A] bg-gray-50 rounded-xl border border-gray-100 active:scale-90 transition-all" 
      ...
      {/* --- MOBILE DROPDOWN (The Drawer) --- */}
      <div className={`md:hidden absolute top-[calc(100%+10px)] left-6 right-6 bg-white rounded-[2rem] border border-gray-100 shadow-[0_25px_60px_rgba(0,0,0,0.15)] transition-all duration-500 ease-[cubic-bezier(0.23,1,0.32,1)] origin-top ${
        ...
        <div className="p-8 flex flex-col gap-1">
          ...
          <div className="mt-6 pt-6 border-t border-gray-50">
            <Button href="/consultation" onClick={closeMenu} className="w-full py-5 text-[11px]">
              Book Consultation
            </Button>
          </div>
        </div>
